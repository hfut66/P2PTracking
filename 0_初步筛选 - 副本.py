...
def parse_args():
    ...
    # 有无模板更新及阈值
    parser.add_argument('--upDatePattern', dest='upDatePattern', default=['No_update', 15, 20, 25])
    parser.add_argument('--upDateLegendName', dest='upDateLegendName', default=['No_update', '15', '20', '25'], help='upDatePattern title')
    ...
    while(True):
        ...
        for ids1,p1 in enumerate(args.boxSize): # 跟踪框大小------6个
            for ids2,p2 in enumerate(args.kpDetect): # 特征检测模式------3个
                ...
                plt.savefig('./实验结果/interval_all_{}_{}.jpg'.format(p1,p2))
                plt.close('all')
                # plt.show()

            # plt.savefig('./实验结果/all_{}.jpg'.format(p1))
            # plt.show()

        print('Result_pattern = ', Result_pattern)
        # 保存字典，便于统计
        file = open('./实验结果/interval_all.txt', 'a')
        file.write(str(Result_pattern)+'\n')
        file.close()
        # 保存可视化图形
        # plt.savefig('./实验结果/all.jpg')
        # plt.show()
        break
